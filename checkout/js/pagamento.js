document.addEventListener('DOMContentLoaded', function() {
  ...
  elPixBox.textContent = pixData.codigo || '';
  if (pixData.qrcodeImagem) elQR.src = pixData.qrcodeImagem;

  // Botão copiar
  btnCopiar.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(elPixBox.textContent.trim());
  ...
  async function checkStatus() {
    ...
    try {
      ...
      if (['paid', 'approved', 'completed', 'confirmed', 'success'].includes(status)) {
        ...
        // Verificar se já foi enviada notificação para evitar duplicatas
        const notificationSentKey = 'pushcut_notification_sent_' + txid;
        if (!localStorage.getItem(notificationSentKey)) {
          // Função para enviar notificações Pushcut (fallback do frontend)
          async function sendPushcutNotification(url) {
            try {
              await fetch(url, { method: 'POST', mode: 'no-cors' });
            } catch (e) {
              console.log('Notificação Pushcut enviada (modo no-cors)');
            }
          }
          
          // Enviar notificações Pushcut quando PIX é pago
          sendPushcutNotification('https://api.pushcut.io/houGcVXDLgJNEIpqnO3Bo/notifications/Venda%20aprovada%20vega');
